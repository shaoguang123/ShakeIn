...
class svm_train {
	...
	public void do_cross_validation()
	{
		   ...
		   param.svm_type == svm_parameter.NU_SVR)
		{
			...
			String m1="Cross Validation Mean squared error = "+total_error/prob.l+"\n";
			String m2="Cross Validation Squared correlation coefficient = "+
				((prob.l*sumvy-sumv*sumy)*(prob.l*sumvy-sumv*sumy))/
				((prob.l*sumvv-sumv*sumv)*(prob.l*sumyy-sumy*sumy))+"\n";
			Log.e("m1", m1);
			Log.e("m2", m2);
		...
		else
		{
			for(i=0;i<prob.l;i++)
				if(target[i] == prob.y[i])
					++total_correct;
			String m3="c:"+param.C+"g:"+param.gamma+"Cross Validation Accuracy = "+100.0*total_correct/prob.l+"%\n";
			Log.e("m3", m3);
	...
	public void my_parse_command_line(String argv[])
	{
		int i;
		svm_print_interface print_func = null;	// default printing to stdout
	...
	public void my_read_problem(ArrayList<Double[]> t_x,ArrayList<Double> t_y) throws IOException
	{
		Vector<Double> vy = new Vector<Double>(t_y);
		Vector<svm_node[]> vx = new Vector<svm_node[]>();
		int max_index = 1200;

		for(int i=0;i<t_x.size();i++)
		{
			svm_node[] x = new svm_node[1200];
			for(int j=0;j<1200;j++)
			{
				x[j] = new svm_node();
				x[j].index = j+1;
				x[j].value = t_x.get(i)[j];
			}
			vx.addElement(x);
		}
	...
	public void my_run() throws IOException
	{
		error_msg = svm.svm_check_parameter(prob,param);
